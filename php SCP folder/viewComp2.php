...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (!empty($startDate) || !empty($endDate)) {
        ...
        if (empty($startDate)) {
            $query = "SELECT * FROM complaints WHERE info1 <= ? AND Forward_To = ? AND Status= 'Accepted'";
        ...
        elseif (empty($endDate)) {
            ...
            $query = "SELECT * FROM complaints WHERE info1 BETWEEN ? AND ? AND Forward_To = ? AND Status= 'Accepted'";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "sss", $startDate, $endDate, $email);
        }
    }
    else{
        $query = "SELECT * FROM complaints WHERE Forward_To = ? AND Status= 'Accepted'";
        $stmt = mysqli_prepare($conn, $query);
